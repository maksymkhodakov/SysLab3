...
void lexRegexAnalyze(const std::string &jsCode, std::ofstream &outfile) {
    ...
    std::regex dotRegex("\\.");
    std::regex numericLiteralRegex("\\b\\d+\\.?\\d*([eE][-+]?\\d+)?\\b");  // For decimal and floating-point numbers
    std::regex hexLiteralRegex("\\b0x[0-9a-fA-F]+\\b");  // For hexadecimal numbers
    ...
    while (!remainingCode.empty()) {
        ...
        std::vector<std::pair<std::regex, std::string>> patterns = {
                ...
                {hexLiteralRegex, "Hexadecimal Literal"},  // Separate entry for hexadecimal literals
